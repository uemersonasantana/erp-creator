        ...
        <?php
          if( !isset($pesquisa_chave) ) {
            if( isset($campos)==false ) {
                if( file_exists($_SERVER['DOCUMENT_ROOT']."/files/dbinputs/db_func_db_syscampo.php") == true ) {
                  include($_SERVER['DOCUMENT_ROOT']."/files/dbinputs/db_func_db_syscampo.php");
                } else {
                  $campos = "db_syscampo.*";
                }
            }
            if( isset($chave_codcam) and (trim($chave_codcam)!="") ) {
               $sql = $cl_syscampo->sql_query($chave_codcam,$campos,"codcam");
            }else if(isset($chave_nomecam) and (trim($chave_nomecam)!="") ){
               $sql = $cl_syscampo->sql_query("",$campos,"nomecam"," nomecam like '$chave_nomecam%' ");
            }else{
               $sql = $cl_syscampo->sql_query("",$campos,"codcam","");
            }
            $db_stdlib->db_lovrot($sql,15,"()","",( isset($funcao_js) ? $funcao_js : null ) );
          }else{
            if( $pesquisa_chave != null and $pesquisa_chave != "" ) {
              $result = $cl_syscampo->sql_record($cl_syscampo->sql_query($pesquisa_chave));
              if($cl_syscampo->numrows!=0){
