...
class UsuarioSistema {
  ...
  /**
   * Caminho para o arquivo JSON contendo as mensagens utilizadas na função db_stdlib::_M
   */
  ...
  public function salvar() {
    ...
    if ($oDaoUsuarioSistema->erro_status == 0) {
      throw new Exception( db_stdlib::_M( UsuarioSistema::MENSAGENS . 'erro_salvar_usuario' ) );
    }
    return true;
  }
  ...
  public function getUsuarioByLogin($sLoginUsuario) {
    ...
    if ($iLinhasBusca == 0) {
      throw new \Exception( db_stdlib::_M( UsuarioSistema::MENSAGENS . 'usuario_nao_localizado' ) );
    }

    $oDadoUsuario = $rsExecutaBusca->fetch();
  ...
  public function getPreferenciasUsuario() {
    $oPreferenciaUsuario = new \model\PreferenciaUsuario($this);
  ...
  private function carregarInstituicoes() {
    ...
    if (count($this->aInstituicoes) == 0) {
      ...
      if ($oDaoUsuarioInstituicao->erro_status == "0") {
        throw new BusinessException( db_stdlib::_M( UsuarioSistema::MENSAGENS . 'nenhuma_instituicao_usuario' ) );
      }
  ...
  public function getDepartamentos() {
    ...
    if (empty($this->aDepartamentos)) {
      ...
      $rsDepartamentos = db_stdlib::db_query($sSqlDepartamentos);

      if (!$rsDepartamentos) {
        throw new Exception( db_stdlib::_M( UsuarioSistema::MENSAGENS . 'erro_departamentos_usuario' ) );
      }
  ...
  public function validaToken( $sToken = null ){
    ...
    if( $sToken != $this->getToken() ){
      throw new Exception( db_stdlib::_M( UsuarioSistema::MENSAGENS . 'token_invalido' ) );
    }
    return true;
  }
  ...
  public function validaPrimeiroAcesso( $sToken = null ){
    ...
    if( $this->getStatusUsuario() != 3  ){
      throw new Exception( db_stdlib::_M( UsuarioSistema::MENSAGENS . 'situacao_invalida_ativacao' ) );
    }

    if( $this->getSenha() != '67a74306b06d0c01624fe0d0249a570f4d093747' ){
      throw new Exception( db_stdlib::_M( UsuarioSistema::MENSAGENS . 'senha_padrao_invalida' ) );
    }

    if( $this->isAdministrador() == 1 ){
      throw new Exception( db_stdlib::_M( UsuarioSistema::MENSAGENS . 'usuario_administrador' ) );
    }

    if( $this->isUsuarioExterno() == 1 ){
      throw new Exception( db_stdlib::_M( UsuarioSistema::MENSAGENS . 'usuario_perfil_invalido' ) );
    }
    ...
    if( !empty($sToken) ){
      ...
      if ( strtotime($this->getDataToken() . " + {$oPreferenciaCliente->getDiasExpiraToken()} days") < time())  {
        throw new Exception( db_stdlib::_M( UsuarioSistema::MENSAGENS . 'token_expirado') );
      }
    }
  ...
  public function   isAtualizado() {
    ...
    $return = (bool) pregdb_stdlib::_Match($regex, $this->sEmail);
